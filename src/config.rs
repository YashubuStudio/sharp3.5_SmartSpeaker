...
pub struct SttConfig {
    ...
    /// 認識言語（例: "ja", "en"）
    pub language: String,
    /// GPU (CUDA) を使用するか（デフォルト: true）
    #[serde(default = "default_use_gpu")]
    pub use_gpu: bool,
    /// Flash Attentionを使用するか（デフォルト: true）
    /// IM2COL failedエラーが出る場合はfalseにする
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        anyhow::ensure!(
            ...
            "audio.sample_rate は正の値である必要があります (現在: {})",
            self.audio.sample_rate
        );
        anyhow::ensure!(
            self.audio.silence_threshold >= 0.0 && self.audio.silence_threshold <= 1.0,
            "audio.silence_threshold は 0.0〜1.0 の範囲である必要があります (現在: {})",
            self.audio.silence_threshold
        );
        anyhow::ensure!(
            self.audio.silence_duration > 0.0,
            "audio.silence_duration は正の値である必要があります (現在: {})",
            self.audio.silence_duration
        );
        anyhow::ensure!(
            self.audio.max_record_seconds > 0.0,
            "audio.max_record_seconds は正の値である必要があります (現在: {})",
            self.audio.max_record_seconds
        );
        anyhow::ensure!(
            self.audio.input_gain > 0.0,
            "audio.input_gain は正の値である必要があります (現在: {})",
            self.audio.input_gain
        );
        anyhow::ensure!(
            self.audio.smoothing_alpha > 0.0 && self.audio.smoothing_alpha <= 1.0,
            "audio.smoothing_alpha は 0.0〜1.0 の範囲である必要があります (現在: {})",
            self.audio.smoothing_alpha
        );

        // wakeword
        anyhow::ensure!(
            self.wakeword.threshold >= 0.0 && self.wakeword.threshold <= 1.0,
            "wakeword.threshold は 0.0〜1.0 の範囲である必要があります (現在: {})",
            self.wakeword.threshold
        );
        anyhow::ensure!(
            self.wakeword.avg_threshold >= 0.0 && self.wakeword.avg_threshold <= 1.0,
            "wakeword.avg_threshold は 0.0〜1.0 の範囲である必要があります (現在: {})",
            self.wakeword.avg_threshold
        );

        // tts
        anyhow::ensure!(
            self.tts.speed >= 0.5 && self.tts.speed <= 2.0,
        ...
        if self.rag.enabled {
            anyhow::ensure!(
                self.rag.similarity_threshold >= 0.0 && self.rag.similarity_threshold <= 1.0,
                "rag.similarity_threshold は 0.0〜1.0 の範囲である必要があります (現在: {})",
                self.rag.similarity_threshold
            );
            anyhow::ensure!(
                self.rag.top_k > 0,
                "rag.top_k は正の値である必要があります (現在: {})",
                self.rag.top_k
            );
        }
